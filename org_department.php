<!doctype html>

<html lang="en">

<head>

  <title>宽柔中学古来分校 | 行政单位 | 行政单位</title>

  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">

  <style>
    .row {
      font-family: sans-serif;
      font-size: small
    }

    .pricing-tables {
      display: flex;
      flex-wrap: wrap;
      justify-content: center;
      gap: 30px 20px;
      padding-top: 60px
    }

    .pricing-table {
      width: 100%;
      min-width: 250px;
      padding: 0 !important;
      cursor: pointer;
    }

    .plan-name {
      width: 100%;
    }
  </style>

</head>

<body>

  <!-- Container -->
  <div id="container">

    <!-- Start Header -->
    <?php
    $header = file_get_contents('header.php');
    echo $header;
    ?>
    <!-- End Header -->


    <!-- Start Page Banner -->
    <div class="page-banner"
      style="background: linear-gradient(to bottom, rgba(0,0,0,0) 50%, rgba(0,0,0,1)), url(img/subbanner04.jpg) center #f9f9f9;">
      <div class="container">
        <div class="col-md-6">
          <h2>行政单位</h2>
        </div>
        <div class="col-md-6">
          <ul class="breadcrumbs">
            <a href="index@a38b.php">
              <li><i>首页 /&ensp;</i></li>
            </a>
            <a href="org_department.php">
              <li><i>行政单位 /&ensp;</i></li>
            </a>
            <a href="">
              <li><i>行政单位</i></li>
            </a>
          </ul>
        </div>
      </div>
    </div>
    <!-- End Page Banner -->


    <!-- Start Content -->
    <div id="content">
      <div class="container">
        <div class="page-content">

          <div class="row">

            <!-- Start Pricing Table Section -->
            <div class="pricing-section pricing-tables">
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_principalsoffice.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od1.jpg">
                  <h3>校长室</h3>
                  <div class="interval">Principal's Office</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>

              <div class="pricing-table col-md-2 col-sm-12" onclick="window.open('department_academic.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od2.jpg">
                  <h3>教务处</h3>
                  <div class="interval">Academic Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>

              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_disciplinary.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od3.jpg">
                  <h3>训导处</h3>
                  <div class="interval">Disciplinary Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>

              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_cocurricular.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od4.jpg">
                  <h3>联课活动处</h3>
                  <div class="interval">Co-curricular Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_generalaffairs.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od5.jpg">
                  <h3>事务处</h3>
                  <div class="interval">General Affairs Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_humanresource.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od6.jpg">
                  <h3>人力资源处</h3>
                  <div class="interval">Human Resource Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12" onclick="window.open('department_sports.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od7.jpg">
                  <h3>体育处</h3>
                  <div class="interval">Sports Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_furtherstudiescounselling.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od8.jpg">
                  <h3>升学辅导处</h3>
                  <div class="interval">Further Studies Counselling Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_counselling.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od9.jpg">
                  <h3>生活辅导处</h3>
                  <div class="interval">Counselling Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_computercentre.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od10.jpg">
                  <h3>电脑中心</h3>
                  <div class="interval">Computer Centre Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_multimediacentre.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od10.jpg">
                  <h3>视听教育馆</h3>
                  <div class="interval">Multimedia Centre Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12" onclick="window.open('department_library.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od12.jpg">
                  <h3>图书馆</h3>
                  <div class="interval">Library</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
              <div class="pricing-table col-md-2 col-sm-12"
                onclick="window.open('department_eveningclass.php', '_self')">
                <div class="plan-name">
                  <img src="img/organization/org_department_icon/od13.jpg">
                  <h3>夜间部</h3>
                  <div class="interval">Evening Class Department</div>
                </div>
                <div class="plan-signup">
                  <a class="btn-system btn-small border-btn">查看更多 >></a>
                </div>
              </div>
            </div>
            <!-- End Pricing Table Section -->

          </div>
        </div>
      </div>
    </div>
    <!-- End content -->


    <!-- Start Footer -->
    <?php
    $footer = file_get_contents('footer.php');
    echo $footer;
    ?>
    <!-- End Footer -->

  </div>
  <!-- End Container -->

  <!-- Go To Top Link -->
  <a href="#" class="back-to-top"><i class="fa fa-angle-up"></i></a>

  <script type="text/javascript" src="js/script.js"></script>

</body>

</html>