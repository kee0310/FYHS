...
<head>
  ...
  <script type="text/javascript">
    function previewMultiple(event) {
      let image = document.getElementById("announcement_image");
...
<body>
  ...
  <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
    ...
    <div class="row">
      ...
      include('connect.php');
      $id = $_GET['id'];
      $result = mysqli_query($conn, "SELECT * FROM wannouncement where announcement_id='$id'");
      while ($row = mysqli_fetch_array($result)) {
        ?>

        <div class="panel panel-default" style="margin: 20px 15px">
          <div class="panel-heading">删除资料 (活动/通知/公告)</div>

          <div class="panel-body">
            <form role="form" action="wannouncement_delete_exe.php" method="post">
              <div class="col-md-6">
                <input type="hidden" name="announcement_id" value="<?php echo $row['announcement_id']; ?>">

                <div class="form-group">
                  <label>发布日期</label>
                  <input disabled class="form-control" type="date" value="<?php echo $row['announcement_date']; ?>"
                    name="announcement_date">
                </div>

                <div class="form-group">
                  <label>发布单位</label>
                  <select class="form-control" name="announcement_dept">
                    <option value="<?php echo $row['announcement_dept']; ?>">
                ...
                <div class="form-group">
                  <label>标题</label>
                  <input disabled class="form-control" value="<?php echo $row['announcement_title']; ?>"
                    name="announcement_title">
                </div>

                <div class="form-group">
                  <label>内容</label>
                  <textarea disabled class="form-control" name="announcement_content" rows="10"
                    cols="50"><?php echo $row['announcement_content']; ?></textarea>
                ...
                <div id="gallery">
                  <?php
                  include('connect.php');
                  $result = mysqli_query($conn, "SELECT * FROM wannouncement_img where announcement_id='$id' ORDER BY img_id DESC");
                  while ($row = mysqli_fetch_array($result)) {
                    echo '<img src="../img/announcement/' . $row['img_name'] . '" alt="">';
                  }
                  ?>
                </div>
